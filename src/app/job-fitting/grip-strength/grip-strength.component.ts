...
export class GripStrengthComponent implements OnInit {
  ...
  getAssociatedTaskInit(workerId:number , taskSetId:number){
    ...
    this.jobFittingService.getGripStrengthTasksSummary(workerId,options).subscribe(result => {
  ...
  getJobFitSummary(workerId:number , jobId:number, associatedId:number){
    ...
    this.jobFittingService.getJobFitSummary(workerId,options).subscribe(result => {
      ...
      if(associatedId){
        ...
        this.getGripStrength(null , this.selectedJobFitSummary);
       }
       this.loaded = true;
    });
  }
  getGripStrength(e:any, summaryInit?:JobFitSummary, filterType?:number){
    this.loaded = false;
    this.optionSelected = true;
    ...
    if(type === 'Job') {
      ...
      this.jobFittingService.getGripStrengthJobsSummary(workerId,options).subscribe(result => {
        this.jobFitItems = result;
        this.jobFitItemsGroups =  result.filter(element => {
          const isDuplicate = uniqueIds.includes(element.groupName);
          if (!isDuplicate) {
  ...
  filterFit(e:any){
    this.getGripStrength(null, this.selectedJobFitSummary, e?.value);
  }
