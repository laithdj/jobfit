...
export class JFSGroupDropDown{
  label:string = '';
  items:JobFitSummary[] = [];
}
@Component({
  selector: 'app-postural-tolerances',
...
export class PosturalTolerancesComponent implements OnInit {
  ...
  getJobFitSummary(workerId:number , jobId:number , associatedId?:number){
    ...
    this.jobFittingService.getJobFitSummary(workerId,options).subscribe(result => {
      ...
      if(associatedId){
        ...
        this.getPosturalTolerances(null , this.selectedJobFitSummary);
      }
      this.loaded = true;
    });
  }
  getAssociatedTaskInit(workerId:number , taskSetId:number){
    ...
    this.loaded = false;
    this.jobFittingService.getPosturalTolerancesTasksSummary(workerId,options).subscribe(result => {
  ...
  getPosturalTolerances(e:any, summaryInit?: JobFitSummary, filterType?:number){
    this.loaded = false; 
    this.optionSelected = true;
    ...
    if(type === 'Job') {
      ...
      this.jobFittingService.getPosturalTolerancesJobsSummary(workerId,options).subscribe(result => {
        this.jobFitItems = result;
        this.jobFitItemsGroups =  result.filter(element => {
          const isDuplicate = uniqueIds.includes(element.groupName);
          if (!isDuplicate) {
  ...
  filterFit(e:any){
    this.getPosturalTolerances(null, this.selectedJobFitSummary, e?.value);
  }
}
