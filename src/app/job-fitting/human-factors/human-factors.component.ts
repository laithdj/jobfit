...
export class HumanFactorsComponent implements OnInit {
  ...
  getAssociatedTaskInit(workerId:number , taskSetId:number){
    ...
    this.response = false;
    this.jobFittingService.getTasksHumanFactorsTree(this.workerId,options).subscribe(result => {
  ...
  filterFit(e:any){
    ...
    if(this.filtered){
      this.selectedHFTree =_.cloneDeep(this.fullList);
      this.filtered = false;
    }
      if(e.value > 1){
        if (this.selectedHFTree) {
          let choice = e.value === 3;
          this.filterHFFit(this.selectedHFTree, choice);
        }
        this.filtered = true;
      }
  ...
  filterHFFit(selectedHFTree: HumanFactorTree[], isFit: boolean)
  {
    if (selectedHFTree.length > 0) {
      for (var i= selectedHFTree.length - 1; i >= 0; i--)
      {
          var currentNode = selectedHFTree[i];
          if (currentNode) {
            // are we a child node
            ...
            else
            {
                ...
                {
                    // recursive
                    this.filterHFFit(currentNode.children, isFit);
  ...
  getJobFitSummary(workerId:number , jobId:number,associatedId:number){
    ...
    this.jobFittingService.getJobFitSummary(workerId,options).subscribe(result => {
      ...
      if(associatedId){
        ...
        this.getHumanFactors(null , this.selectedJobFitSummary);
       }
      });
  }


  getHumanFactors(e:any, summaryInit?:JobFitSummary){
    this.response = false;
    ...
    if(item?.isJob) {
      this.jobFittingService.getHumanFactorTreeSummary(this.workerId , options).subscribe(result => {
        this.selectedHFTree =_.cloneDeep(result);
