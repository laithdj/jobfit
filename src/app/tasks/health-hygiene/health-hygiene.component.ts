...
import { HealthHygieneNextAssessmentSchedule } from 'src/app/shared/models/task.model';
...
export class HealthHygieneComponent implements OnInit {
  ...
  constructor(
    ...
    this.route.params.subscribe((params: Params) => {
      ...
      if (this.authorisedFunctionList) {
        if (this.authorisedFunctionList.Function[EFunctions.ViewHealthHygienes] && this.authorisedFunctionList.Function[EFunctions.ViewTasks]) {
          ...
          this.store.dispatch(new FetchTask(this.taskId));
            ...
            this.getProviders();
            this.getMainHygieneResults(this.taskId);
            this.getNextAssessmentSchedules(this.taskId);
        } else {
          this.empLoaded = true;
  ...
  getMainHygieneList() {
    this.HHLoaded = false;
    this.taskService.getMainHealthHygieneByType("Tasks").subscribe((results: any) => {
  ...
  getMainHygieneResults(id: number) {
    this.HHLoaded = false;
    this.taskService.getMainHealthHygieneResults(id).subscribe((results: HealthHygieneResult[]) => {
  ...
  getNextAssessmentSchedules(id: number) {
    this.taskService.getTaskHealthHygieneNextAssessmentSchedule(id).subscribe((results: HealthHygieneNextAssessmentSchedule[]) => {
  ...
  saveResult() {
    if(this.assessmentDate == null){
      ...
      this.taskService.saveMainHealthHygieneResult(this.taskId , this.selectedHealthHygiene).subscribe((results: any) => {
  ...
  viewEdit(hyg: any) {
    ...
    this.header = "View/Edit Health Hygiene Result Details";
  ...
  archiveNextAssessment(){
    ...
    if (id> 0) {
      this.taskService.archiveHealthHygieneNextAssessmentSchedule(id).subscribe((results:any) => {
        if(results){
  ...
  saveNextAssessment(){
    if (this.nasDate == null){
      ...
      this.displayError = true;
        ...
        this.taskService.saveTaskHealthHygieneNextAssessmentSchedule(this.taskId, this.nextAssessmentSchedule).subscribe((results: any) => {
    ...
    archiveBtnHH(hyg: any, functionId:number){
      this.showArchiveHH = true;
      this.selectedHealthHygieneResult = hyg
    }
}
