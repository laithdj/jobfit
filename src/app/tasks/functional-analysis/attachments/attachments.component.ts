...
export class FAAttachmentsComponent implements OnInit {
  ...
  getAttachments(faId:number, page:number , count:number){
    ...
    this.tasksService.getAttachments(faId,this.attachmentCriteria).subscribe(result => {
  ...
  saveAttachment(){
    if (this.attachment.displayFileName == ""){
      ...
      this.tasksService.saveFAAttachments(this.faId,this.attachment).subscribe(result => {
        if(result){
